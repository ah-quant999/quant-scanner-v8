window.HEALTH_CHECK = {
  "updated": "2026-08-17 10:36:32",
  "overall": "fail",
  "summary": {
    "ok": 36,
    "warn": 3,
    "fail": 9,
    "total": 48
  },
  "items": [
    {
      "id": "V8_CAL",
      "name": "重要事件日历",
      "page": "今日事件",
      "freq": "每周日+月末",
      "status": "ok",
      "last_update": "2026-08-17 08:19:34",
      "age_min": 137.0,
      "message": "更新于 今日 08:19"
    },
    {
      "id": "IPO_DATA",
      "name": "打新研判",
      "page": "今日事件",
      "freq": "每日盘前",
      "status": "ok",
      "last_update": "2026-08-17 08:18:32",
      "age_min": 138.0,
      "message": "更新于 今日 08:18"
    },
    {
      "id": "JUDGMENT_DATA",
      "name": "今日判定",
      "page": "今日事件",
      "freq": "每日盘前",
      "status": "ok",
      "last_update": "2026-08-17 08:19:36",
      "age_min": 136.9,
      "message": "更新于 今日 08:19"
    },
    {
      "id": "MACRO_DATA",
      "name": "今日宏观解读",
      "page": "今日事件",
      "freq": "每日盘前",
      "status": "ok",
      "last_update": "2026-08-17 08:18:43",
      "age_min": 137.8,
      "message": "更新于 今日 08:18"
    },
    {
      "id": "NT_DATA",
      "name": "市场提示",
      "page": "今日事件",
      "freq": "每日盘前",
      "status": "ok",
      "last_update": "2026-08-17 07:53",
      "age_min": 163.5,
      "message": "更新于 今日 07:53",
      "heal_cat": "algo_run"
    },
    {
      "id": "INDEX_QUOTES",
      "name": "全球指数 / 股指期货",
      "page": "实时数据",
      "freq": "盘中每30分",
      "status": "fail",
      "last_update": "2026-08-17 08:19:36",
      "age_min": 0,
      "premarket_cleared": true,
      "message": "盘中交易时段被异常标记为 premarket_cleared（update_time=今日 08:19）"
    },
    {
      "id": "ETF_PULSE",
      "name": "ETF 盘中异动",
      "page": "实时数据",
      "freq": "盘中实时",
      "status": "fail",
      "last_update": "2026-08-17 08:19:36",
      "age_min": 0,
      "premarket_cleared": true,
      "message": "盘中交易时段被异常标记为 premarket_cleared（update_time=今日 08:19）"
    },
    {
      "id": "ETF_INTRADAY_HEAT",
      "name": "ETF 资金热度",
      "page": "实时数据",
      "freq": "盘中实时 T+0",
      "status": "fail",
      "last_update": "2026-08-17 08:19:36",
      "age_min": 0,
      "premarket_cleared": true,
      "message": "盘中交易时段被异常标记为 premarket_cleared（update_time=今日 08:19）"
    },
    {
      "id": "ETF_DAILY_MONITOR",
      "name": "ETF 日监控",
      "page": "实时数据",
      "freq": "盘中每30分",
      "status": "fail",
      "last_update": "2026-08-17 06:38:16",
      "age_min": 238.3,
      "message": "更新于 今日 06:38；超过阈值 45 分钟"
    },
    {
      "id": "SECTOR_FUND_FLOW",
      "name": "板块资金流向",
      "page": "实时数据",
      "freq": "盘中每30分",
      "status": "fail",
      "last_update": "2026-08-17 08:19:36",
      "age_min": 0,
      "premarket_cleared": true,
      "message": "盘中交易时段被异常标记为 premarket_cleared（update_time=今日 08:19）"
    },
    {
      "id": "CONCEPT_RANKING",
      "name": "概念排名",
      "page": "实时数据",
      "freq": "盘中每30分",
      "status": "fail",
      "last_update": "2026-08-17 08:19:36",
      "age_min": 0,
      "premarket_cleared": true,
      "message": "盘中交易时段被异常标记为 premarket_cleared（update_time=今日 08:19）"
    },
    {
      "id": "LIMIT_UP_HEATMAP",
      "name": "涨停热度",
      "page": "实时数据",
      "freq": "盘中每30分",
      "status": "fail",
      "last_update": "2026-08-17 08:19:36",
      "age_min": 0,
      "premarket_cleared": true,
      "message": "盘中交易时段被异常标记为 premarket_cleared（update_time=今日 08:19）"
    },
    {
      "id": "MARKET_FUND_FLOW_DATA",
      "name": "市场资金流向",
      "page": "实时数据",
      "freq": "盘中每30分",
      "status": "fail",
      "last_update": "2026-08-17 08:19:33",
      "age_min": 137.0,
      "message": "更新于 今日 08:19；超过阈值 45 分钟"
    },
    {
      "id": "MARKET_ALERTS",
      "name": "市场预警",
      "page": "实时数据",
      "freq": "盘中实时",
      "status": "fail",
      "last_update": "2026-08-17 08:19:36",
      "age_min": 0,
      "premarket_cleared": true,
      "message": "盘中交易时段被异常标记为 premarket_cleared（update_time=今日 08:19）"
    },
    {
      "id": "SH_FIB",
      "name": "市场温度计",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 07:38",
      "age_min": 178.5,
      "message": "更新于 今日 07:38",
      "heal_cat": "algo_run"
    },
    {
      "id": "SIX_DIM_RADAR",
      "name": "六维共振雷达",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 07:38",
      "age_min": 178.5,
      "message": "更新于 今日 07:38",
      "heal_cat": "algo_run"
    },
    {
      "id": "MARGIN_DATA",
      "name": "融资融券",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 08:18:32",
      "age_min": 138.0,
      "message": "更新于 今日 08:18"
    },
    {
      "id": "CFFEX_HOLDINGS",
      "name": "股指期货持仓",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 08:18:33",
      "age_min": 138.0,
      "message": "更新于 今日 08:18"
    },
    {
      "id": "CRISIS_DATA",
      "name": "危机雷达",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 08:18:52",
      "age_min": 137.7,
      "message": "更新于 今日 08:18"
    },
    {
      "id": "MARKET_FUND_FLOW_DATA",
      "name": "盘后资金流向",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 08:19:33",
      "age_min": 137.0,
      "message": "更新于 今日 08:19"
    },
    {
      "id": "CANDIDATE",
      "name": "候选池",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-16 19:25:59",
      "age_min": 910.5,
      "message": "更新于 昨日 19:25",
      "heal_cat": "algo_run"
    },
    {
      "id": "GOLD_POOL",
      "name": "黄金池",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 07:54:57",
      "age_min": 161.6,
      "message": "更新于 今日 07:54",
      "heal_cat": "algo_run"
    },
    {
      "id": "LHB_DATA",
      "name": "龙虎榜",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 07:39:45",
      "age_min": 176.8,
      "message": "更新于 今日 07:39",
      "heal_cat": "algo_run"
    },
    {
      "id": "INST_TRADE",
      "name": "机构买卖",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 07:38:13",
      "age_min": 178.3,
      "message": "更新于 今日 07:38",
      "heal_cat": "algo_run"
    },
    {
      "id": "TRIPLE_CONSENSUS",
      "name": "三重共识",
      "page": "盘后数据",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 07:52:39",
      "age_min": 163.9,
      "message": "更新于 今日 07:52",
      "heal_cat": "algo_run"
    },
    {
      "id": "FOUR_VOLUME",
      "name": "四量终极",
      "page": "选股策略",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 10:26:59",
      "age_min": 9.5,
      "message": "更新于 今日 10:26",
      "heal_cat": "algo_run"
    },
    {
      "id": "COCKPIT_ADVICE",
      "name": "驾驶舱",
      "page": "选股策略",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-16 15:27:38",
      "age_min": 1148.9,
      "message": "更新于 昨日 15:27",
      "heal_cat": "algo_run"
    },
    {
      "id": "BIG_BULL_HUNTER",
      "name": "大牛股猎手",
      "page": "选股策略",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 07:39:45",
      "age_min": 176.8,
      "message": "更新于 今日 07:39",
      "heal_cat": "algo_run"
    },
    {
      "id": "TOP10_DAILY",
      "name": "全站精选",
      "page": "选股策略",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 07:45:06",
      "age_min": 171.4,
      "message": "更新于 今日 07:45",
      "heal_cat": "algo_run"
    },
    {
      "id": "STOCK_RPS",
      "name": "相对强度",
      "page": "选股策略",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 09:26:10",
      "age_min": 70.4,
      "message": "更新于 今日 09:26",
      "heal_cat": "algo_run"
    },
    {
      "id": "CRDS_CARD_DATA",
      "name": "逆势龙头",
      "page": "选股策略",
      "freq": "收盘后1次",
      "status": "ok",
      "last_update": "2026-08-17 07:39:54",
      "age_min": 176.6,
      "message": "更新于 今日 07:39",
      "heal_cat": "algo_run"
    },
    {
      "id": "raw_etf_pulse.json",
      "name": "raw_data/etf_pulse.json",
      "page": "管线",
      "status": "ok",
      "message": "存在"
    },
    {
      "id": "raw_capital_flow_data.json",
      "name": "raw_data/capital_flow_data.json",
      "page": "管线",
      "status": "ok",
      "message": "存在"
    },
    {
      "id": "raw_index_quotes.json",
      "name": "raw_data/index_quotes.json",
      "page": "管线",
      "status": "ok",
      "message": "存在"
    },
    {
      "id": "raw_crisis_data.json",
      "name": "raw_data/crisis_data.json",
      "page": "管线",
      "status": "ok",
      "message": "存在"
    },
    {
      "id": "raw_concept_ranking.json",
      "name": "raw_data/concept_ranking.json",
      "page": "管线",
      "status": "ok",
      "message": "存在"
    },
    {
      "id": "site_sync",
      "name": "Pages 部署同步",
      "page": "管线",
      "status": "ok",
      "message": "本地 HEAD 5b41483 / 线上 03976ef 已同步（线上落后 1 commit ≤5，Pages 异步部署正常）"
    },
    {
      "id": "runner_local",
      "name": "runner 本地检测",
      "page": "管线",
      "status": "ok",
      "message": "LEMONCAT 最近抓取 2026-08-17 08:19:36，14 个模块全部成功"
    },
    {
      "id": "runner_github",
      "name": "runner GitHub API 检测",
      "page": "管线",
      "status": "ok",
      "message": "最近运行正常（本地状态: LEMONCAT 最近抓取 2026-08-17 08:19:36，14 个模块全部成功）"
    },
    {
      "id": "runner",
      "name": "self-hosted runner",
      "page": "管线",
      "status": "ok",
      "message": "runner 本地检测:ok-LEMONCAT 最近抓取 2026-08-17 08:19:36，14 个模块全部成功 | runner GitHub API 检测:ok-最近运行正常（本地状态: LEMONCAT 最近抓取 2026-08-17 08:19:36，14 个模块全部成功）"
    },
    {
      "id": "local_sync",
      "name": "本地与 origin/main 同步",
      "page": "管线",
      "status": "ok",
      "message": "本地 5b41483 / origin/main 5b41483 同步"
    },
    {
      "id": "cockpit_signal_stale",
      "name": "驾驶舱建议信号日期",
      "page": "内容审计",
      "status": "ok",
      "message": "生成时间 2026-08-16 15:27:24，最新信号发生日期 --（历史回测样本，非陈旧指标）"
    },
    {
      "id": "final_enter_stale",
      "name": "最终推荐入选日期",
      "page": "内容审计",
      "status": "ok",
      "message": "最新 enter_date 20260814 ≥ 最近交易日 2026-08-14"
    },
    {
      "id": "final_market_distribution",
      "name": "最终推荐市场分布",
      "page": "内容审计",
      "status": "ok",
      "message": "Top5 市场分布：A股5只 / 港股0只"
    },
    {
      "id": "top10_history_depth",
      "name": "T+N 信号台账深度",
      "page": "内容审计",
      "status": "ok",
      "message": "top10_daily 历史跨度 51 个交易日 (2026-06-06 ~ 2026-08-17)，满足 T+20 跟踪需求"
    },
    {
      "id": "a_share_triple_consensus",
      "name": "三重共识 A股覆盖",
      "page": "内容审计",
      "heal_cat": "algo_run",
      "status": "warn",
      "message": "⚠️ 三重共识 今日 0 只（交易日无数据，可能弱市或上游故障；若其他池也 0 则排查上游；若此池独立为 0 而其他池正常则不报警）"
    },
    {
      "id": "a_share_cockpit_tier_recommend",
      "name": "驾驶舱分档 A股覆盖",
      "page": "内容审计",
      "heal_cat": "algo_run",
      "status": "warn",
      "message": "⚠️ 驾驶舱分档 今日 0 只（交易日无数据，可能弱市或上游故障；若其他池也 0 则排查上游；若此池独立为 0 而其他池正常则不报警）"
    },
    {
      "id": "a_share_mahoro",
      "name": "国际投行信号 A股覆盖",
      "page": "内容审计",
      "heal_cat": "algo_run",
      "status": "warn",
      "message": "⚠️ 国际投行信号 今日 0 只（交易日无数据，可能弱市或上游故障；若其他池也 0 则排查上游；若此池独立为 0 而其他池正常则不报警）"
    }
  ]
};
