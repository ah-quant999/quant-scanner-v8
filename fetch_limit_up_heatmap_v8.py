...
def _check_dates_integrity(existing_dates):
    """检查已有日期序列是否完整覆盖最近15个交易日，防止丢列/错列。"""
    if not existing_dates or len(existing_dates) < 15:
        return False, f"日期不足15列 ({len(existing_dates)})"
    if len(existing_dates) != len(set(existing_dates)):
        return False, "存在重复日期"
    expected = get_trade_dates(15)
    ...
    if extra:
        return False, f"包含非最近15日日期 {sorted(extra)}"
    return True, "OK"
...
def main():
    ...
    parser.add_argument("--rebuild", action="store_true", help="强制全量重建近15个交易日")
    ...
    if need_rebuild:
        # ── 全量重建：拉取近15个交易日 ──
        trade_dates = get_trade_dates(15)
        ...
        # 防御：剔除「15日全 0 且今日仍 0」的板块（米业/国产电池这类历史遗留不会复活）
        ...
        if today_str in ed:
            ...
            new_dates = (ed[-14:] + [today_str]) if len(ed) >= 14 else (ed + [today_str])
        new_dates = new_dates[-15:]
