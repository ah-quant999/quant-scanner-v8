...
# -*- coding: utf-8 -*-
"""
backtest_allsite.py — 全站精选（final_recommend 推荐池）历史回测

背景：
  final_recommend.py 现已按日归档 raw_data/history/final_recommend_YYYYMMDD.json
  （2026-09-02 起生效）。本脚本读取这些 dated 快照做真实前向收益回测。

本脚本：
  1. 读取 raw_data/history/final_recommend_YYYYMMDD.json 历史推荐快照。
  2. 取每日 top_n 推荐 stocks（stocks 为空则回退 all_candidates）作为信号（信号日收盘价买入）。
  3. 用 baostock 拉取信号股日K线，计算 T+1/T+3/T+5/T+10/T+20 持有期收益。
  4. 输出 raw_data/allsite_backtest.json + data/ALLSITE_BACKTEST.js。

输出字段（与 CRDS_BACKTEST / HUNTER_BACKTEST 同构）：
  - summary: update_time, total_signals, calc_time, method, signal_date_range,
             by_period {1/3/5/10/20: {samples, win_rate, avg_return, ...}}
  - signals: 逐信号明细

使用：
  python v8/backtest_allsite.py
  python v8/backtest_allsite.py --dry        # 只统计信号数，不拉K线

注意：
  - baostock 登录失败时自动降级为空回测（避免 CI 挂死）。
  - 历史文件时间戳为文件名后 8 位 yyyymmdd。
"""
import argparse
import json
import sys
...
def parse_date_from_filename(name):
    """final_recommend_20260902.json -> 2026-09-02"""
    stem = Path(name).stem
...
def load_signals():
    """加载历史 final_recommend 推荐信号。"""
    signals = []
    seen = set()
    if not HISTORY_DIR.exists():
        return signals
    for f in sorted(HISTORY_DIR.glob("final_recommend_*.json")):
        try:
        ...
        if not stocks:
            stocks = data.get("all_candidates") or []
        for s in stocks:
...
def main():
    ...
    print(f"[allsite backtest] loaded {len(signals)} recommend signals")
    if not signals:
        payload = empty_backtest("无历史 final_recommend 信号（每日归档自 2026-09-02 起生效，待累积 dated 快照）")
        OUT_JSON.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        OUT_JS.write_text("window.ALLSITE_BACKTEST = " + json.dumps(payload, ensure_ascii=False, indent=1) + ";", encoding="utf-8")
        print("[allsite backtest] written empty backtest")
        return 0
    ...
    if not baostock_login():
        ...
        OUT_JSON.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        OUT_JS.write_text("window.ALLSITE_BACKTEST = " + json.dumps(payload, ensure_ascii=False, indent=1) + ";", encoding="utf-8")
        print("[allsite backtest] baostock login failed -> empty backtest")
        return 0
    ...
    payload = {
        ...
        "summary": {
            ...
            "calc_time": now,
            "method": "全站精选历史回测：推荐日收盘价买入，持有N个交易日收盘价卖出（不处理节假日，按日历日+1/3/5/10/20）",
    ...
    OUT_JSON.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    OUT_JS.write_text("window.ALLSITE_BACKTEST = " + json.dumps(payload, ensure_ascii=False, indent=1) + ";", encoding="utf-8")
    print(f"[allsite backtest] done: {len(signals)} signals, periods={ {p: len(period_returns[p]) for p in HOLD_PERIODS} }")
    return 0
