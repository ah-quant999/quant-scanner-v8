...
# 体积上限（2026-08-08 新增，2026-09-16 重新标定）：防止数据被内联回 index.html 导致巨型化。
# 标定值遵循原哲学 = 「当前真值 × 2 倍余量」：
#   2026-08-08 真值≈680KB → 1.25MB；2026-09-16 真值≈1.20MB（1,262,734 字节，全部为内联
#   JS 代码、data/*.js 仍经 <script src> 外置，非数据内联）→ 2.5MB。
# 上次 6.8MB 巨型化的根因就是某次部署把 data/*.js 内联进 index.html，
# 而旧版护栏只查下界不查上界，反而"越大越通过"放行了。
# 2.5MB 上限仍可拦截 6.8MB 内联事故（2.7× 余量），并为轻量化收尾期的代码增减留足空间。
