...
## 九、第二轮 UI 批（15:04 推完 `756297d011b2`，只改 `index.html` 一个文件）

### 9.1 主人令 → 落点

| 主人原话 | 改动 |
|---|---|
| 「这里改成一行，短线写在前，中长写在后，要提亮标识」 | AI市场速览「盘面观测（短线·实时）」与「仓位依据（中线·回测）」**两行合一**，**短线在前、中线在后**，中间竖线 `\|` 分隔；尺度标识由 10px 灰字改为**各自语义色的实心小胶囊**（提亮）；调和语仍在下一行 |
| 「第二行的中性观望不需要了…把平均股价超买超卖那个金色胶囊找回来，放到第一行后面」 | 删「⏸ 中性·观望」（中性态零信息量）；超买超卖（位置观测）金色胶囊由第二行**移至第一行末尾**；第二行改**条件渲染**（仅「卖/减」或「破 MA20/破 MA60」时出现）⇒ 中性态整卡自然收成一行 |

数据源、配色语义、结论文字、买卖信号逻辑、闸门约束**一律未动**（7 项「未误伤」断言把关）。

### 9.2 验证与推后核验（全绿）

- 补丁 `994247 → 994742` 字符（+495）；自检含「**逆运算逐字节还原基线**」⇒ 证明只做定点替换，其余一字节未动。
- Node vm 真实渲染 **61/61**（从产物抽真实代码段执行）；HTML 审计全绿（内联 script 28 块语法 0 错、注入清单 94→94、div 平衡 0 差）。
- 推后远端真值 **19/19**；线上 `index.html` = **994742 字符**，10/10（`v8_boot_sec` 与两个四量占位注入**仍在** ⇒ 前两批成果未被顶掉）。
- CI run `34939875252`：**step9 `Pre-deploy audit` ✅ / step10 ✅ / step11「📤 部署到 main」✅（非 skipped）** ⇒ 构建部署链保持解锁。

### 9.3 🔴 新踩到的坑（已修，工具已固化）

**Git Data API 的 `PATCH /git/refs/heads/main` 返回 422 = 非 fast-forward**。成因：在「建 blob → tree → commit」这几秒里 main 又被云端推进（本仓 `index.html` 每 ~2 分钟有缓存戳任务在改，实测 13:00→15:04 之间 tip 漂了 5 次以上）。

首次执行时该异常**直接冒泡中断**了推仓（15:00:15 那次）。已改为：**422/409 视为「并发冲突」信号 → 返回 None → 外层重拉 base 重试**（轮数 10→20），**绝不 force 覆盖他人**。

⇒ 凡在本仓用 Git Data API 做单文件推仓，都必须如此处理；单纯加大 `tries` 无用（非 ff 不会因重试自愈，必须换新 base）。

### 9.4 给阿狸咪

- `index.html` 今日已改多轮（11:20 P0 批 / 12:30 两卡改造 / 15:04 本批），**任何本地缓存基线都已失效**，必须运行时从 main tip 重拉。
- 15:10 那个「UI二轮推送」任务已改为**只核验**（15:40 跑），**别再重复推**。
