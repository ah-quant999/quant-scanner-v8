...
WARN_SOURCES = {
    ...
    "SUSPENSION_ALERT": 48,
    "MARKET_ALERTS": 48,
    # ⏱ 2026-09-21（本批）小九：**本值即 v8_health_check.py::_LOW_FREQ_MAX_AGE["STOCK_LIST"]
    #   的同源口径**。改本值必须同批改另一处 —— 两守卫口径不一致本身就是 bug
    #   （同一份数据一绿一红；实测 2026-09-21：本处 30 天 / 健康检查 24h
    #    ⇒ STOCK_LIST 每周一必被健康检查判 fail）。
    #   🔴 量纲不同，**切勿「对齐数值」**：本处单位 = **小时**（24*30 = 720h = 30 天）；
    #      健康检查处单位 = **分钟**（30*24*60 = 43200min = 30 天）。数值不同、语义等价。
    "STOCK_LIST": 24 * 30,
