...
## 🔴🔴 主人工作守则（2026-09-23 主人令 · 正式入协议 · 双机所有会话/专家通用）

> 来源：主人 2026-09-23 原话 ——「**写到双机新的那个协议里去，不要我每个专家都说一遍**」。
> 本节对**所有专家、所有会话、两台机**一体生效：任何会话开工读到此节即视为已被告知，
> **无需主人对每个专家重复声明**，也不得以「没轮到我」为由豁免。

**1. 提问边界（只有一种情况才问）**
只有**不确定主人的意思和意图**时，才问主人的意见和建议；
其余情形一律用专家的科学方法自主行事 ——
**发现问题就直接一劳永逸修复，修好后审计没问题就直接上线**
（与 2026-09-21「修好+审计完直接上线」常驻授权同源，此处重申并入协议）。

**2. 邮件纪律（告警闸门）**
不要一直发邮件 —— **只能发「不能自愈、不能解决、必须主人出面」的邮件**。
进度汇报 / 自愈成功 / 已自行解决 / 计划内重试中 / 误报复核通过 —— **一律不发**。

**3. 三件套（每笔改动收口，缺一不可）**
① **审计** —— 改完必审计（源码级断言 ＋ 线上真值验证，不以 run 结论代替）；
② **四方对齐** —— 云端、双机、运维页、逻辑详解页四处口径一致（含卡名/任务/producer 可反查）；
③ **推仓防覆盖 ＋ 交接** —— 推仓入库走防覆盖（expect-base / 基线断言，绝不 force），
   并按本协议与另一台机交接（改 `HANDOFF.yaml` 状态字段，不写回执档）。

**4. 全程说中文（与主人及所有专家交互、所有交付文档一律中文）**
所有与主人及专家的对话、所有交付文档 / 报告 / 交接件 / 代码注释中的说明文字，**一律使用简体中文**；
代码标识符（变量名、函数名、分支名等）可保留英文，**但禁止在中文叙述里夹带英文混答**、
禁止用英文写交付结论或状态汇报。本守则与用户级长期记忆「🔴🔴 全程简体中文」同源，此处并入协议，双机一律执行。
