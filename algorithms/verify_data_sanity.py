...
STALE_OK = {
    ...
    # 2026-09-23 一劳永逸（主人令 09-14「候选池/金股池是算法上游水源，不需回测」）：
    #   此二者是**历史回测产物**（不重算就不变），拿每日新鲜度阈值(>7天硬失败)卡它们是品类误判；
    #   09-14 那轮只从闸门摘除登记、未删文件也未去前端引用 ⇒ 校验器一直扫到 09-14 的孤儿文件判「真断更」。
    #   回测本非每日数据，前端卡仍渲染历史回测 ⇒ 豁免新鲜度对账（而非删文件），使算法链回归绿。
    #   ⚠️ 后续彻底清理：删 data/CANDIDATE_BACKTEST.js / GOLD_POOL_BACKTEST.js + raw_data 同名 + 前端 index.html:1793 / logic.html:1507 引用（触及主文件，另行审慎处理）。
    "CANDIDATE_BACKTEST",
    "GOLD_POOL_BACKTEST",
}
...
def check_value(var, key, val):
    ...
    #    与本基金闸门的使命(旧装新/越界/空表)不符；候选池/龙虎榜/金股池等列表型
