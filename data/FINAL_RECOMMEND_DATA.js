window.FINAL_RECOMMEND_DATA = {
  "update_time": "2026-09-24 03:08:38",
  "crisis_score": 32.1,
  "crisis_high": false,
  "crisis_note": "危机雷达未达高位，逆势龙头暂不并入",
  "total_candidates": 20,
  "top_n": 5,
  "data_degraded": true,
  "signal_edge": {
    "source": "backtest_expectancy@2026-09-24 02:20:20",
    "degraded": false,
    "effective": {
      "jinzuan": 1.051,
      "chan": 0.889,
      "trend": -1.964,
      "jigou": -1.037
    },
    "hardcoded_default": {
      "jinzuan": 0.532,
      "chan": 0.759,
      "trend": -1.532,
      "jigou": -0.66
    },
    "n_on10": {
      "jinzuan": 151,
      "chan": 304,
      "trend": 248,
      "jigou": 405
    },
    "consistent": {
      "jinzuan": true,
      "chan": false,
      "trend": true,
      "jigou": true
    },
    "loaded": {
      "generated": "2026-09-24 02:20:20",
      "hit": 4,
      "n_snapshots": 70,
      "date_range": [
        "2026-06-06",
        "2026-09-24"
      ]
    },
    "alpha_select": {
      "enabled": true,
      "skipped_no_positive_edge": 0
    }
  },
  "degrade_note": "⚠️ 数据降级：因子实验室（FACTOR_LAB.js）未达数据日（实际 data_date=2026-09-23） ⇒ 方案B 因子融合整体跳过。本结果为「降级放行」产物 —— 排序与信号有效，但上述环节未达最新口径。请对照产物内 factor_chain / signal_edge 元数据判断适用范围。",
  "market_regime": {
    "date": "2026-09-23",
    "regime": "grind",
    "open": true,
    "ok": true,
    "reason": "ok",
    "note": "grind/panic=可开仓(正常推)；stabilize/rebound=历史回测≥3共振负期望，应观察/少推；ok=false 表示 regime 取数失败已按保守处置（regime=null、不推满仓）"
  },
  "price_source": {
    "date": "2026-09-24",
    "source": "CANDIDATE_QUOTES",
    "covered": 10,
    "total": 20,
    "note": "价格唯一权威来源（当日有效快照）；未覆盖的票 close/pct_chg 为 null，不伪造 0.00%"
  },
  "strong_sectors": [
    "互联网电商",
    "化学制药",
    "医疗器械",
    "医疗服务",
    "半导体",
    "家居用品",
    "小家电",
    "小金属",
    "房地产",
    "生物制品",
    "贵金属",
    "非金属材料"
  ],
  "stocks": [
    {
      "rank": 1,
      "code": "300199",
      "name": "翰宇药业",
      "market": "sz",
      "board": "创业板",
      "horizon": "短线/中线共振",
      "close": 24.29,
      "close_date": "2026-09-24",
      "close_source": "CANDIDATE_QUOTES",
      "close_verified": true,
      "pct_chg": 1.84,
      "stop_loss": 21.86,
      "target_price": 27.7,
      "risk_reward": 1.4,
      "support": 22.26,
      "resistance": 24.44,
      "atr": 0.91,
      "sources": [
        "三重共识",
        "四量终极"
      ],
      "source_scores": {
        "三重共识": 1.34,
        "四量终极": 1.47
      },
      "resonance": 2,
      "strength": 2.81,
      "sector_score": 0.0,
      "sector_hits": [],
      "sector_fund": [],
      "final_score": 33.5,
      "buy_score": 33.5,
      "enter_date": "2026-09-22",
      "signals": [
        "机构变红",
        "缠论买点",
        "跨策略共振"
      ],
      "_60m_resonance": false,
      "reason": "三重共识 评分34；四量终极 信号2项",
      "industry": "医药制造业",
      "concepts": [
        "流感",
        "病原体防治",
        "新消费",
        "辅助生殖",
        "小盘股",
        "AI制药（医疗）",
        "减肥药",
        "互联医疗"
      ],
      "tracking": {
        "entry_date": "2026-09-22",
        "entry_price": 24.29,
        "latest_price": 24.29,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪",
        "alerts": [
          {
            "level": "good",
            "code": "300199",
            "name": "翰宇药业",
            "text": "翰宇药业(300199) 连续 3 日稳居严格共识（高质量）"
          }
        ]
      },
      "action": "买入",
      "market_regime": "grind"
    },
    {
      "rank": 2,
      "code": "002463",
      "name": "沪电股份",
      "market": "sz",
      "board": "主板",
      "horizon": "短线",
      "close": 128.1,
      "close_date": "2026-09-24",
      "close_source": "CANDIDATE_QUOTES",
      "close_verified": true,
      "pct_chg": 2.37,
      "stop_loss": 115.29,
      "target_price": 142.99,
      "risk_reward": 1.16,
      "support": 110.59,
      "resistance": 131.25,
      "atr": 6.35,
      "sources": [
        "四量终极"
      ],
      "source_scores": {
        "四量终极": 1.47
      },
      "resonance": 1,
      "strength": 1.47,
      "sector_score": 0.0,
      "sector_hits": [],
      "sector_fund": [],
      "final_score": 25.5,
      "buy_score": 25.5,
      "enter_date": "2026-09-24",
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "_60m_resonance": false,
      "reason": "四量终极 信号2项",
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "国产芯片",
        "新能源车",
        "深成500",
        "昨日高振幅",
        "5G概念",
        "百元股"
      ],
      "tracking": {
        "entry_date": "2026-09-24",
        "entry_price": 128.1,
        "latest_price": 128.1,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪"
      },
      "action": "买入",
      "market_regime": "grind"
    },
    {
      "rank": 3,
      "code": "000703",
      "name": "FUTURE BRIGHT",
      "market": "sz",
      "board": "主板",
      "horizon": "短线",
      "close": 17.39,
      "close_date": "2026-09-24",
      "close_source": "四量终极",
      "close_verified": true,
      "pct_chg": 6.78,
      "stop_loss": 15.65,
      "target_price": 20.13,
      "risk_reward": 1.58,
      "support": 15.88,
      "resistance": 19.37,
      "atr": 0.86,
      "sources": [
        "四量终极"
      ],
      "source_scores": {
        "四量终极": 1.47
      },
      "resonance": 1,
      "strength": 1.47,
      "sector_score": 0.0,
      "sector_hits": [],
      "sector_fund": [],
      "final_score": 24.8,
      "buy_score": 24.8,
      "enter_date": "2026-09-24",
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "_60m_resonance": false,
      "reason": "四量终极 信号2项",
      "industry": "化学纤维制造业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "昨日高振幅",
        "深股通",
        "2026中报预增",
        "转债标的",
        "西部大开发",
        "中盘股"
      ],
      "tracking": {
        "entry_date": "2026-09-24",
        "entry_price": 17.39,
        "latest_price": 17.39,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪",
        "alerts": [
          {
            "level": "good",
            "code": "000703",
            "name": "恒逸石化",
            "text": "恒逸石化(000703) 连续 3 日稳居严格共识（高质量）"
          },
          {
            "level": "warn",
            "code": "000703",
            "name": "恒逸石化",
            "text": "恒逸石化(000703) 入选以来回撤 -13.5%（2026-08-20 起）"
          }
        ]
      },
      "action": "买入",
      "market_regime": "grind"
    },
    {
      "rank": 4,
      "code": "688808",
      "name": "联讯仪器",
      "market": "sh",
      "board": "科创板",
      "horizon": "短线",
      "close": 2280.02,
      "close_date": "2026-09-24",
      "close_source": "CANDIDATE_QUOTES",
      "close_verified": true,
      "pct_chg": -1.55,
      "stop_loss": 2052.02,
      "target_price": 2733.0,
      "risk_reward": 1.99,
      "support": 2180.0,
      "resistance": 2733.0,
      "atr": 146.9,
      "sources": [
        "四量终极"
      ],
      "source_scores": {
        "四量终极": 1.5
      },
      "resonance": 1,
      "strength": 1.5,
      "sector_score": 0.0,
      "sector_hits": [],
      "sector_fund": [],
      "final_score": 24.8,
      "buy_score": 24.8,
      "enter_date": "2026-09-24",
      "signals": [
        "四量终极共振"
      ],
      "_60m_resonance": false,
      "reason": "四量终极 信号0项",
      "industry": "仪器仪表制造业",
      "concepts": [
        "仪器仪表制造业"
      ],
      "tracking": {
        "entry_date": "2026-09-24",
        "entry_price": 2280.02,
        "latest_price": 2280.02,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪",
        "alerts": [
          {
            "level": "warn",
            "code": "688808",
            "name": "联讯仪器",
            "text": "联讯仪器(688808) 于 2026-09-23 跌出共识（曾连续 2 日）"
          }
        ]
      },
      "action": "买入",
      "market_regime": "grind"
    },
    {
      "rank": 5,
      "code": "300502",
      "name": "新易盛",
      "market": "sz",
      "board": "创业板",
      "horizon": "短线",
      "close": 451.2,
      "close_date": "2026-09-24",
      "close_source": "CANDIDATE_QUOTES",
      "close_verified": true,
      "pct_chg": -0.84,
      "stop_loss": 406.08,
      "target_price": 579.42,
      "risk_reward": 2.84,
      "support": 378.56,
      "resistance": 475.0,
      "atr": 21.78,
      "sources": [
        "四量终极"
      ],
      "source_scores": {
        "四量终极": 1.31
      },
      "resonance": 1,
      "strength": 1.31,
      "sector_score": 0.0,
      "sector_hits": [],
      "sector_fund": [],
      "final_score": 23.0,
      "buy_score": 23.0,
      "enter_date": "2026-09-24",
      "signals": [
        "机构变红"
      ],
      "_60m_resonance": false,
      "reason": "四量终极 信号1项",
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "富时罗素",
        "西部大开发",
        "深成500",
        "算力概念",
        "权重股",
        "5G概念",
        "东方财富热股",
        "CPO概念"
      ],
      "tracking": {
        "entry_date": "2026-09-24",
        "entry_price": 451.2,
        "latest_price": 451.2,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪"
      },
      "action": "买入",
      "market_regime": "grind"
    }
  ],
  "consensus_stocks": [
    {
      "rank": 1,
      "code": "300199",
      "name": "翰宇药业",
      "market": "深市",
      "board": "创业板",
      "horizon": "短线/中线共振",
      "close": 24.29,
      "close_date": "2026-09-24",
      "close_source": "CANDIDATE_QUOTES",
      "close_verified": true,
      "pct_chg": 1.84,
      "stop_loss": 21.86,
      "target_price": 27.7,
      "risk_reward": 1.4,
      "support": 22.26,
      "resistance": 24.44,
      "atr": 0.91,
      "sources": [
        "三重共识",
        "四量终极"
      ],
      "source_scores": {
        "三重共识": 1.34,
        "四量终极": 1.47
      },
      "resonance": 2,
      "strength": 2.81,
      "final_score": 33.5,
      "sector_score": 0.0,
      "sector_hits": [],
      "enter_date": "2026-09-22",
      "signals": [
        "跨策略共振",
        "缠论买点",
        "机构变红"
      ],
      "_60m_resonance": false,
      "reason": "三重共识 评分34；四量终极 信号2项",
      "industry": "医药制造业",
      "concepts": [
        "流感",
        "病原体防治",
        "新消费",
        "辅助生殖",
        "小盘股",
        "AI制药（医疗）",
        "减肥药",
        "互联医疗"
      ],
      "tracking": {
        "entry_date": "2026-09-22",
        "entry_price": 24.29,
        "latest_price": 24.29,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪"
      },
      "action": "买入",
      "market_regime": "grind"
    },
    {
      "rank": 2,
      "code": "002463",
      "name": "沪电股份",
      "market": "深市",
      "board": "主板",
      "horizon": "短线",
      "close": 128.1,
      "close_date": "2026-09-24",
      "close_source": "CANDIDATE_QUOTES",
      "close_verified": true,
      "pct_chg": 2.37,
      "stop_loss": 115.29,
      "target_price": 142.99,
      "risk_reward": 1.16,
      "support": 110.59,
      "resistance": 131.25,
      "atr": 6.35,
      "sources": [
        "四量终极"
      ],
      "source_scores": {
        "四量终极": 1.47
      },
      "resonance": 1,
      "strength": 1.47,
      "final_score": 25.5,
      "sector_score": 0.0,
      "sector_hits": [],
      "enter_date": "2026-09-24",
      "signals": [
        "缠论买点",
        "机构变红"
      ],
      "_60m_resonance": false,
      "reason": "四量终极 信号2项",
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "国产芯片",
        "新能源车",
        "深成500",
        "昨日高振幅",
        "5G概念",
        "百元股"
      ],
      "tracking": {
        "entry_date": "2026-09-24",
        "entry_price": 128.1,
        "latest_price": 128.1,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪"
      },
      "action": "买入",
      "market_regime": "grind"
    },
    {
      "rank": 3,
      "code": "000703",
      "name": "FUTURE BRIGHT",
      "market": "深市",
      "board": "主板",
      "horizon": "短线",
      "close": 17.39,
      "close_date": "2026-09-24",
      "close_source": "四量终极",
      "close_verified": true,
      "pct_chg": 6.78,
      "stop_loss": 15.65,
      "target_price": 20.13,
      "risk_reward": 1.58,
      "support": 15.88,
      "resistance": 19.37,
      "atr": 0.86,
      "sources": [
        "四量终极"
      ],
      "source_scores": {
        "四量终极": 1.47
      },
      "resonance": 1,
      "strength": 1.47,
      "final_score": 24.8,
      "sector_score": 0.0,
      "sector_hits": [],
      "enter_date": "2026-09-24",
      "signals": [
        "缠论买点",
        "机构变红"
      ],
      "_60m_resonance": false,
      "reason": "四量终极 信号2项",
      "industry": "化学纤维制造业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "昨日高振幅",
        "深股通",
        "2026中报预增",
        "转债标的",
        "西部大开发",
        "中盘股"
      ],
      "tracking": {
        "entry_date": "2026-09-24",
        "entry_price": 17.39,
        "latest_price": 17.39,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪"
      },
      "action": "买入",
      "market_regime": "grind"
    },
    {
      "rank": 4,
      "code": "688808",
      "name": "联讯仪器",
      "market": "沪市",
      "board": "科创板",
      "horizon": "短线",
      "close": 2280.02,
      "close_date": "2026-09-24",
      "close_source": "CANDIDATE_QUOTES",
      "close_verified": true,
      "pct_chg": -1.55,
      "stop_loss": 2052.02,
      "target_price": 2733.0,
      "risk_reward": 1.99,
      "support": 2180.0,
      "resistance": 2733.0,
      "atr": 146.9,
      "sources": [
        "四量终极"
      ],
      "source_scores": {
        "四量终极": 1.5
      },
      "resonance": 1,
      "strength": 1.5,
      "final_score": 24.8,
      "sector_score": 0.0,
      "sector_hits": [],
      "enter_date": "2026-09-24",
      "signals": [],
      "_60m_resonance": false,
      "reason": "四量终极 信号0项",
      "industry": "仪器仪表制造业",
      "concepts": [
        "仪器仪表制造业"
      ],
      "tracking": {
        "entry_date": "2026-09-24",
        "entry_price": 2280.02,
        "latest_price": 2280.02,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪"
      },
      "action": "买入",
      "market_regime": "grind"
    },
    {
      "rank": 5,
      "code": "300502",
      "name": "新易盛",
      "market": "深市",
      "board": "创业板",
      "horizon": "短线",
      "close": 451.2,
      "close_date": "2026-09-24",
      "close_source": "CANDIDATE_QUOTES",
      "close_verified": true,
      "pct_chg": -0.84,
      "stop_loss": 406.08,
      "target_price": 579.42,
      "risk_reward": 2.84,
      "support": 378.56,
      "resistance": 475.0,
      "atr": 21.78,
      "sources": [
        "四量终极"
      ],
      "source_scores": {
        "四量终极": 1.31
      },
      "resonance": 1,
      "strength": 1.31,
      "final_score": 23.0,
      "sector_score": 0.0,
      "sector_hits": [],
      "enter_date": "2026-09-24",
      "signals": [
        "机构变红"
      ],
      "_60m_resonance": false,
      "reason": "四量终极 信号1项",
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "富时罗素",
        "西部大开发",
        "深成500",
        "算力概念",
        "权重股",
        "5G概念",
        "东方财富热股",
        "CPO概念"
      ],
      "tracking": {
        "entry_date": "2026-09-24",
        "entry_price": 451.2,
        "latest_price": 451.2,
        "return_pct": 0.0,
        "hold_days": 1,
        "exit_type": "hold",
        "note": "今日新入选，自动开始跟踪"
      },
      "action": "买入",
      "market_regime": "grind"
    }
  ],
  "all_candidates": [
    {
      "code": "300199",
      "name": "翰宇药业",
      "market": "深市",
      "board": "创业板",
      "horizon": "中长线",
      "close": 24.29,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 1.84,
      "final_score": 33.5,
      "resonance": 2,
      "sources": [
        "三重共识",
        "四量终极"
      ],
      "signals": [
        "机构变红",
        "缠论买点",
        "跨策略共振"
      ],
      "industry": "医药制造业",
      "concepts": [
        "流感",
        "病原体防治",
        "新消费",
        "辅助生殖",
        "小盘股",
        "AI制药（医疗）",
        "减肥药",
        "互联医疗"
      ],
      "enter_date": "2026-09-22",
      "stop_loss": 21.86,
      "target_price": 27.7,
      "risk_reward": 1.4,
      "support": 22.26,
      "resistance": 24.44,
      "factor_actions": []
    },
    {
      "code": "002463",
      "name": "沪电股份",
      "market": "深市",
      "board": "主板",
      "horizon": "短线",
      "close": 128.1,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 2.37,
      "final_score": 25.5,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "国产芯片",
        "新能源车",
        "深成500",
        "昨日高振幅",
        "5G概念",
        "百元股"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 115.29,
      "target_price": 142.99,
      "risk_reward": 1.16,
      "support": 110.59,
      "resistance": 131.25,
      "factor_actions": []
    },
    {
      "code": "000703",
      "name": "FUTURE BRIGHT",
      "market": "深市",
      "board": "主板",
      "horizon": "短线",
      "close": 17.39,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 6.78,
      "final_score": 24.8,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "industry": "化学纤维制造业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "昨日高振幅",
        "深股通",
        "2026中报预增",
        "转债标的",
        "西部大开发",
        "中盘股"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 15.65,
      "target_price": 20.13,
      "risk_reward": 1.58,
      "support": 15.88,
      "resistance": 19.37,
      "factor_actions": []
    },
    {
      "code": "688808",
      "name": "联讯仪器",
      "market": "沪市",
      "board": "科创板",
      "horizon": "短线",
      "close": 2280.02,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": -1.55,
      "final_score": 24.8,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [],
      "industry": "仪器仪表制造业",
      "concepts": [
        "仪器仪表制造业"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 2052.02,
      "target_price": 2733.0,
      "risk_reward": 1.99,
      "support": 2180.0,
      "resistance": 2733.0,
      "factor_actions": []
    },
    {
      "code": "300502",
      "name": "新易盛",
      "market": "深市",
      "board": "创业板",
      "horizon": "短线",
      "close": 451.2,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": -0.84,
      "final_score": 23.0,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红"
      ],
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "富时罗素",
        "西部大开发",
        "深成500",
        "算力概念",
        "权重股",
        "5G概念",
        "东方财富热股",
        "CPO概念"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 406.08,
      "target_price": 579.42,
      "risk_reward": 2.84,
      "support": 378.56,
      "resistance": 475.0,
      "factor_actions": []
    },
    {
      "code": "600309",
      "name": "万华化学",
      "market": "沪市",
      "board": "主板",
      "horizon": "短线",
      "close": 71.6,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 1.33,
      "final_score": 23.0,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [],
      "industry": "化学原料和化学制品制造业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "上证180",
        "煤化工概念",
        "证金持股",
        "大盘价值",
        "锂电池概念",
        "茅指数"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 64.44,
      "target_price": 78.47,
      "risk_reward": 0.96,
      "support": 69.1,
      "resistance": 78.47,
      "factor_actions": []
    },
    {
      "code": "000807",
      "name": "上海实业环境",
      "market": "深市",
      "board": "主板",
      "horizon": "短线",
      "close": 26.58,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 5.6,
      "final_score": 23.0,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "上涨趋势",
        "机构变红"
      ],
      "industry": "有色金属冶炼和压延加工业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "西部大开发",
        "新能源车",
        "深成500",
        "大盘价值",
        "燃料电池概念",
        "融资融券"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 23.92,
      "target_price": 29.13,
      "risk_reward": 0.96,
      "support": 26.14,
      "resistance": 29.13,
      "factor_actions": []
    },
    {
      "code": "688498",
      "name": "源杰科技",
      "market": "沪市",
      "board": "科创板",
      "horizon": "短线",
      "close": 1734.98,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": -1.03,
      "final_score": 21.7,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红"
      ],
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "半导体概念",
        "国产芯片",
        "光纤概念",
        "西部大开发",
        "昨日高振幅",
        "上证380",
        "百元股",
        "融资融券"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 1561.48,
      "target_price": 1919.0,
      "risk_reward": 1.06,
      "support": 1440.03,
      "resistance": 1888.0,
      "factor_actions": []
    },
    {
      "code": "601168",
      "name": "西部矿业",
      "market": "沪市",
      "board": "主板",
      "horizon": "短线",
      "close": 36.81,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 1.13,
      "final_score": 21.7,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "上涨趋势",
        "机构变红"
      ],
      "industry": "有色金属矿采选业",
      "concepts": [
        "标准普尔",
        "上证180",
        "西部大开发",
        "黄金概念",
        "昨日高振幅",
        "钒电池",
        "东方财富热股",
        "融资融券"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 33.13,
      "target_price": 43.29,
      "risk_reward": 1.76,
      "support": 34.67,
      "resistance": 41.97,
      "factor_actions": []
    },
    {
      "code": "688183",
      "name": "生益电子",
      "market": "沪市",
      "board": "科创板",
      "horizon": "短线",
      "close": 119.0,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 0.69,
      "final_score": 21.1,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "缠论买点"
      ],
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "无人驾驶",
        "上证180",
        "2026中报预增",
        "HS300",
        "PCB",
        "MSCI中国",
        "大盘成长",
        "沪股通"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 107.1,
      "target_price": 138.47,
      "risk_reward": 1.64,
      "support": 109.1,
      "resistance": 138.47,
      "factor_actions": []
    },
    {
      "code": "300037",
      "name": "新宙邦",
      "market": "深市",
      "board": "创业板",
      "horizon": "短线",
      "close": 72.37,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 1.44,
      "final_score": 21.1,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "富时罗素",
        "半导体概念",
        "特斯拉概念",
        "转债标的",
        "新能源车",
        "深成500",
        "氟化工概念",
        "锂电池概念"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 65.13,
      "target_price": 94.36,
      "risk_reward": 3.04,
      "support": 60.55,
      "resistance": 76.45,
      "factor_actions": []
    },
    {
      "code": "000737",
      "name": "湾区发展",
      "market": "深市",
      "board": "主板",
      "horizon": "短线",
      "close": 14.82,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 4.84,
      "final_score": 21.1,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "industry": "有色金属冶炼和压延加工业",
      "concepts": [
        "深股通",
        "昨日高振幅",
        "稀缺资源",
        "2026中报预增",
        "中盘成长",
        "PCB",
        "央国企改革",
        "中盘股"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 13.34,
      "target_price": 17.2,
      "risk_reward": 1.61,
      "support": 13.72,
      "resistance": 17.2,
      "factor_actions": []
    },
    {
      "code": "688630",
      "name": "芯碁微装",
      "market": "沪市",
      "board": "科创板",
      "horizon": "短线",
      "close": 440.68,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 3.04,
      "final_score": 21.1,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "上涨趋势",
        "机构变红"
      ],
      "industry": "专用设备制造业",
      "concepts": [
        "半导体概念",
        "专精特新",
        "中盘成长",
        "2026一季报预增",
        "PCB",
        "先进封装",
        "上证380",
        "先进制造风格"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 396.61,
      "target_price": 525.0,
      "risk_reward": 1.91,
      "support": 366.58,
      "resistance": 448.88,
      "factor_actions": []
    },
    {
      "code": "002975",
      "name": "博杰股份",
      "market": "深市",
      "board": "主板",
      "horizon": "短线",
      "close": 110.57,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 10.0,
      "final_score": 19.3,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "industry": "专用设备制造业",
      "concepts": [
        "AI眼镜",
        "机器人概念",
        "富时罗素",
        "MLCC",
        "半导体概念",
        "消费电子概念",
        "特斯拉概念",
        "苹果概念"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 99.51,
      "target_price": 131.4,
      "risk_reward": 1.88,
      "support": 92.8,
      "resistance": 113.02,
      "factor_actions": []
    },
    {
      "code": "600183",
      "name": "生益科技",
      "market": "沪市",
      "board": "主板",
      "horizon": "短线",
      "close": 143.56,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 0.08,
      "final_score": 17.4,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [],
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "上证180",
        "存储芯片",
        "昨日高振幅",
        "5G概念",
        "百元股",
        "融资融券"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 129.2,
      "target_price": 166.21,
      "risk_reward": 1.58,
      "support": 133.07,
      "resistance": 157.5,
      "factor_actions": []
    },
    {
      "code": "002916",
      "name": "深南电路",
      "market": "深市",
      "board": "主板",
      "horizon": "短线",
      "close": 414.77,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 3.69,
      "final_score": 16.8,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "上涨趋势",
        "机构变红"
      ],
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "国产芯片",
        "苹果概念",
        "存储芯片",
        "深成500",
        "中特估",
        "昨日高振幅"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 373.29,
      "target_price": 468.29,
      "risk_reward": 1.29,
      "support": 328.21,
      "resistance": 424.04,
      "factor_actions": []
    },
    {
      "code": "688690",
      "name": "纳微科技",
      "market": "沪市",
      "board": "科创板",
      "horizon": "短线",
      "close": 49.21,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 3.38,
      "final_score": 16.8,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "industry": "化学原料和化学制品制造业",
      "concepts": [],
      "enter_date": "2026-09-24",
      "stop_loss": 44.29,
      "target_price": 50.26,
      "risk_reward": 0.21,
      "support": 35.01,
      "resistance": 50.26,
      "factor_actions": []
    },
    {
      "code": "000630",
      "name": "隽泰控股",
      "market": "深市",
      "board": "主板",
      "horizon": "短线",
      "close": 6.12,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 0.46,
      "final_score": 16.8,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "industry": "有色金属冶炼和压延加工业",
      "concepts": [
        "标准普尔",
        "富时罗素",
        "深成500",
        "大盘价值",
        "黄金概念",
        "5G概念",
        "锂电池概念",
        "机构重仓"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 5.51,
      "target_price": 7.0,
      "risk_reward": 1.44,
      "support": 5.98,
      "resistance": 6.9,
      "factor_actions": []
    },
    {
      "code": "600989",
      "name": "宝丰能源",
      "market": "沪市",
      "board": "主板",
      "horizon": "短线",
      "close": 22.58,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": 2.13,
      "final_score": 16.8,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "上涨趋势",
        "机构变红",
        "缠论买点"
      ],
      "industry": "化学原料和化学制品制造业",
      "concepts": [
        "富时罗素",
        "上证180",
        "煤化工概念",
        "2026中报预增",
        "HS300",
        "MSCI中国",
        "周期股",
        "沪股通"
      ],
      "enter_date": "2026-09-24",
      "stop_loss": 20.32,
      "target_price": 25.58,
      "risk_reward": 1.33,
      "support": 22.47,
      "resistance": 25.58,
      "factor_actions": []
    },
    {
      "code": "688400",
      "name": "凌云光",
      "market": "沪市",
      "board": "科创板",
      "horizon": "短线",
      "close": 47.44,
      "close_date": "2026-09-24",
      "close_verified": true,
      "pct_chg": -0.11,
      "final_score": 16.2,
      "resonance": 1,
      "sources": [
        "四量终极"
      ],
      "signals": [
        "机构变红",
        "缠论买点"
      ],
      "industry": "计算机、通信和其他电子设备制造业",
      "concepts": [],
      "enter_date": "2026-09-24",
      "stop_loss": 42.7,
      "target_price": 73.9,
      "risk_reward": 5.58,
      "support": 42.44,
      "resistance": 49.4,
      "factor_actions": []
    }
  ],
  "factor_chain": {
    "update_time": "2026-09-24 03:08:38",
    "score_formula": "final_score = 因子榜 TOP10_DAILY.total_score（2026-09-20 唯一口径：第二套 final_score 公式已整段清除）",
    "where": "algorithms/final_recommend.py :: 方案B 因子融合",
    "gate": {
      "V8_FUSION_NOISE_FILTER": 1,
      "regime_open": true
    },
    "integrated": [],
    "skipped": true,
    "skip_reason": "本轮 FACTOR_LAB.js 缺失/内容陈旧 ⇒ 方案B 因子融合整体跳过（数据降级）"
  },
  "factor_trace": []
};