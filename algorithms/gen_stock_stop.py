...
def collect_universe():
    ...
    if gp:
        for k, s in (gp.get("stocks", {}) or {}).items():
            add(s)
        # 🔴 2026-09-11 一劳永逸（主人令「发现了就马上一劳永逸式修复」）：**当时**形态为
        #   GOLD_POOL lite 化后 stocks 为空壳（total_count=0，_lite_note「仅保留 latest
        #   聚合」）、真实成员在 candidates（540 只，键形如 sh_600487，值含
        #   code/market/board_label）。
        #   旧实现只读 stocks → 宇宙退化到 FOUR_VOLUME 残余 5 只 → 全部取数失败拒写盘。
        # 🔵 2026-09-13 实测更正（小九勘误，**仅改注释、不改任何逻辑**）：现状已**反转** ——
        #   GOLD_POOL.js 的 stocks 有 253 条真实成员、candidates 为空。
        #   ⇒ 上面两处（stocks + candidates）**都读**才是正确形态：成员落哪一侧都能取到，
        #     只读单侧会随 lite 开关漂移而退化。
